...
pub fn list_devices(c: &Connection, adapter_path: &String) -> Result<Vec<String>, Box<dyn Error>> {
    list_item(c, DEVICE_INTERFACE, adapter_path, "Adapter")
}

pub fn list_services(c: &Connection, device_path: &String) -> Result<Vec<String>, Box<dyn Error>> {
...
pub fn list_descriptors(c: &Connection, device_path: &String) -> Result<Vec<String>, Box<dyn Error>> {
...
fn list_item(
    ...
    for y in z {
        let (path, interfaces) = y.inner().unwrap();
        let x: &[MessageItem] = interfaces.inner().unwrap();
        // println!("Janitha {:?}", path);

        for interface in x {
            let (i, _) = interface.inner().unwrap();
            let name: &str = i.inner().unwrap();
            // println!("Janitha {:?}", name);
